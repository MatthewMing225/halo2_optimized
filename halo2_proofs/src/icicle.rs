...
use crate::{
    arithmetic::FftGroup,
    plonk::{Calculation, CalculationInfo, ValueSource},
...
use icicle_core::{
    ...
    vec_ops::{
        add_scalars, inv_scalars, mul_scalars, scalar_add, scalar_mul, scalar_sub, sub_scalars,
        VecOpsConfig,
    },
};
use icicle_runtime::{
...
pub fn try_load_and_set_backend_device(device_type: &str) {
    let mut selected = device_type;

    if device_type != "CPU" {
        if let Err(err) = icicle_runtime::runtime::load_backend_from_env_or_default() {
            warn!(
                "Failed to load Icicle backend for device {}: {:?}. Falling back to CPU.",
                device_type, err
            );
            selected = "CPU";
        }
    ...
    if selected != "CPU" {
        info!("Setting device {}", selected);
        let device = icicle_runtime::Device::new(selected, 0 /* =device_id*/);
        match set_device(&device) {
            Ok(_) => {
                if let Err(err) = set_default_device(&device) {
                    warn!(
                        "Failed to set default device {}: {:?}. Falling back to CPU.",
                        selected, err
                    );
                } else {
                    return;
            ...
            Err(err) => {
                warn!(
                    "Failed to set device {}: {:?}. Falling back to CPU.",
                    selected, err
                );
            }
        }
    ...
    info!("Setting device CPU");
    let cpu_device = icicle_runtime::Device::new("CPU", 0 /* =device_id*/);
    set_device(&cpu_device).unwrap();
    set_default_device(&cpu_device).unwrap();
}
...
pub fn should_use_cpu_fft(size: usize) -> bool {
    size <= (1
        << u8::from_str_radix(
            &env::var("ICICLE_SMALL_K_FFT").unwrap_or("2".to_string()),
            10,
        )
        .unwrap())
}
...
pub fn create_calculation_data<C: CurveAffine>(
    ...
    for (i, info) in calculations.iter().enumerate() {
        ...
        let (first, second) = match &info.calculation {
            Calculation::Add(a, b) | Calculation::Sub(a, b) | Calculation::Mul(a, b) => (a, b),
...
/// unsafe flatten
pub fn unsafe_flatten<C: CurveAffine>(
    poly_vec: &[Polynomial<C::Scalar, ExtendedLagrangeCoeff>],
) -> Vec<C::ScalarExt> {
...
pub fn create_gate_data<C: CurveAffine>(
    ...
    let icicle_instance =
        device_vec_from_poly_vec::<C, ExtendedLagrangeCoeff>(instance, &IcicleStream::default());
    let icicle_advice =
        device_vec_from_poly_vec::<C, ExtendedLagrangeCoeff>(advice, &IcicleStream::default());
...
pub fn device_vec_from_poly_vec<C: CurveAffine, B>(
    ...
    for poly in poly_vec {
        ...
        let icicle_values: &[ScalarField] =
            unsafe { std::slice::from_raw_parts(values.as_ptr() as *const ScalarField, len) };

        let h_icicle_values = HostSlice::from_slice(icicle_values);
        device_vec[offset..offset + len]
            .copy_from_host_async(h_icicle_values, stream)
            .unwrap();
        offset += len;
    }
...
pub fn device_vec_from_c_scalars<G: Field>(
    coeffs: &[G],
    stream: &IcicleStream,
) -> DeviceVec<ScalarField> {
    ...
    let mut icicle_scalars =
        DeviceVec::<ScalarField>::device_malloc_async(coeffs.len(), stream).unwrap();
    icicle_scalars
...
pub fn c_scalars_from_device_vec<G: PrimeField>(
    device_vec: &mut DeviceVec<ScalarField>,
    stream: &IcicleStream,
) -> Vec<G> {
...
pub fn multiexp_on_device<C: CurveAffine>(
    coeffs: &[C::Scalar],
    bases: &DeviceSlice<Affine<CurveCfg>>,
    stream: &IcicleStream,
) -> C::Curve {
...
/// inplace mul
pub fn inplace_mul(
    device_vec: &mut DeviceSlice<ScalarField>,
    other: &DeviceSlice<ScalarField>,
    stream: &IcicleStream,
) {
    let mut cfg = VecOpsConfig::default();
    cfg.stream_handle = stream.into();
...
/// inplace scalar add
pub fn inplace_scalar_add(
    device_vec: &mut DeviceSlice<ScalarField>,
    scalar: &DeviceSlice<ScalarField>,
    stream: &IcicleStream,
) {
    let mut cfg = VecOpsConfig::default();
    cfg.stream_handle = stream.into();
