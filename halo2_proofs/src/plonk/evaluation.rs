            ...
            .enumerate()
        {
            ...
            let (instance, advice) = join(
                || {
                    ...
                    let mut converted = Vec::with_capacity(instance.len());
                    for poly in instance.iter() {
                        converted.push(domain.coeff_to_extended(poly, &stream));
                    }
                ...
                || {
                    let mut stream = IcicleStream::create().unwrap();
                    let mut converted = Vec::with_capacity(advice.len());
                    for poly in advice.iter() {
                        converted.push(domain.coeff_to_extended(poly, &stream));
                    }
                    stream.synchronize().unwrap();
                    stream.destroy().unwrap();
