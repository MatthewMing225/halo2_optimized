...
impl StreamGuard {
    fn new() -> Self {
        Self {
            stream: IcicleStream::create().unwrap(),
        }
    }

    fn as_mut(&mut self) -> &mut IcicleStream {
        &mut self.stream
    }
}
...
impl Drop for StreamGuard {
    fn drop(&mut self) {
        // Ensure any pending kernels are complete before tearing the stream down.
...
impl<C: CurveAffine> Evaluator<C> {
    ...
    pub(in crate::plonk) fn evaluate_h(
            ...
            .enumerate()
        {
            ...
            let convert_coeffs = |polys: &[Polynomial<C::Scalar, Coeff>]| {
                polys
                    .par_iter()
            ...
            let (instance, advice) = join(
                || convert_coeffs(instance),
                || convert_coeffs(advice),
            );
