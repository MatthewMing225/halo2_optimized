...
use icicle_core::{
    gate_ops::{
        gate_evaluation, lookups_constraint, CalculationData, GateData, GateOpsConfig, HornerData,
        LookupConfig, LookupData,
    },
    traits::FieldImpl,
...
impl FftStream {
    fn new() -> Self {
        let stream = IcicleStream::create().expect("failed to create FFT stream");
        FftStream { stream }
    }

    fn coeff_to_extended<C: CurveAffine>(
        &mut self,
        domain: &EvaluationDomain<C::ScalarExt>,
        poly: &Polynomial<C::ScalarExt, Coeff>,
    ) -> Polynomial<C::ScalarExt, ExtendedLagrangeCoeff> {
        let result = domain.coeff_to_extended(poly, &self.stream);
        self.stream.synchronize().unwrap();
        result
    }
}

impl Drop for FftStream {
    fn drop(&mut self) {
        if let Err(err) = self.stream.synchronize() {
            log::error!("failed to synchronize FFT stream during drop: {err:?}");
        }
        if let Err(err) = self.stream.destroy() {
            log::error!("failed to destroy FFT stream: {err:?}");
        }
    }
}

fn convert_batch_to_extended<C: CurveAffine>(
    domain: &EvaluationDomain<C::ScalarExt>,
    ...
    polys
        .par_iter()
        .map_init(FftStream::new, |stream, poly| {
            stream.coeff_to_extended::<C>(domain, poly)
        })
        .collect()
}
...
pub struct EvaluateHOutput<C: CurveAffine> {
    pub host_poly: Polynomial<C::ScalarExt, ExtendedLagrangeCoeff>,
    pub device_values: DeviceVec<ScalarField>,
}
    ...
    pub(in crate::plonk) fn evaluate_h(
            ...
            .enumerate()
        {
            ...
            let (instance, advice) = {
                ...
                join(
                    || convert_batch_to_extended::<C>(domain, instance_slice),
                    || convert_batch_to_extended::<C>(domain, advice_slice),
                )
            };
            ...
            let num_instance_cols = if num_instance_rows > 0 {
                instance[0].len()
            } else {
                0
            };
            let num_advice_cols = if num_advice_rows > 0 {
                advice[0].len()
            } else {
                0
            };
            ...
            #[cfg(feature = "mv-lookup")]
            {
                ...
                let mut icicle_previous_value =
                    device_vec_from_c_scalars(&values, &IcicleStream::default());
            ...
            // Shuffle constraints
            {
                for (n, shuffle) in shuffles.iter().enumerate() {
                    let product_coset = pk
                        .vk
                        .domain
                        .coeff_to_extended(&shuffle.product_poly, &IcicleStream::default());
        ...
        let device_values = device_vec_from_c_scalars(values.as_ref(), &IcicleStream::default());
        log::info!(
            "evaluation::evaluate_h: completed in {:.3?}",
            total_start.elapsed()
        );
        EvaluateHOutput {
            host_poly: values,
