    ...
    <Scheme::Scalar as PrimeField>::Repr: Eq + Clone + AsRef<[u8]> + Send + Sync,
{
    ...
    fn zero_advice_columns<F: Field>(
        storage: &mut [Polynomial<Assigned<F>, LagrangeCoeff>],
        columns: &[usize],
    ) {
        for &index in columns {
            if let Some(poly) = storage.get_mut(index) {
                parallelize(&mut *poly, |chunk, _| {
        ...
        for (current_phase, column_indices) in phase_column_indices.iter() {
                ...
                let mut phase_assignments = (0..circuits.len())
                    ...
                    .map(move |circuit_index| -> Result<PhaseAssignment<Scheme::Scalar>, Error> {
                        ...
                        let mut advice_storage = {
                            ...
                            zero_advice_columns(&mut guard, &column_indices);
            ...
            #[cfg(not(feature = "parallel-synthesis"))]
            {
                ...
                for (circuit_index, circuit) in circuits.iter().enumerate() {
                    ...
                    zero_advice_columns(advice_buffer, &column_indices);
